...
$(LOCAL_PACKAGE_NAME)_has_kt_sources := $(LOCAL_HAS_KT)
...
if [[ "$($($<)_has_kt_sources)" = "true" ]]; then \
	echo "    ext.kotlin_version = '1.2.71'" >> $($($<)_gradle_buildfile); \
fi
...
if [[ "$($($<)_has_kt_sources)" = "true" ]]; then \
	echo "        classpath 'org.jetbrains.kotlin:kotlin-gradle-plugin:\$$kotlin_version'" >> $($($<)_gradle_buildfile); \
fi
echo "    }" >> $($($<)_gradle_buildfile)
echo "}" >> $($($<)_gradle_buildfile)
echo "" >> $($($<)_gradle_buildfile)
